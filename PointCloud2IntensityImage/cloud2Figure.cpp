...
*/
bool cloud2Figure::readLasFile(const string& file_name, const pcXYZIPtr& point_cloud, Bounds& bound_3d)
{
	std::ifstream ifs;
	...
	bound_3d.min_x = header.GetMinX();
	bound_3d.min_y = header.GetMinY();
	bound_3d.min_z = header.GetMinZ();
	bound_3d.max_x = header.GetMaxX();
	bound_3d.max_y = header.GetMaxY();
	bound_3d.max_z = header.GetMaxZ();
	// 获取点云外包围盒的中心点坐标
	...
	{
		point_cloud->points[i].x = reader.GetPoint().GetX() - center_x;
		point_cloud->points[i].y = reader.GetPoint().GetY() - center_y;
		point_cloud->points[i].z = reader.GetPoint().GetZ();
		point_cloud->points[i].intensity = reader.GetPoint().GetIntensity();
		bound_3d.max_intensity = max(bound_3d.max_intensity, (int)reader.GetPoint().GetIntensity());
		i++;
	}
	// 检查 强度信息是否存在
